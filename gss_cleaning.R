...
raw_data <- read_csv("AAs7XzS9.csv")
...
gss <- raw_data %>% 
  select(CASEID, 
         ...
         nmarevrc,
         nsepevr) %>% 
  mutate_at(vars(agec:nsepevr), .funs = funs(ifelse(.>=96, NA, .))) %>% 
  mutate_at(.vars = vars(evermar:wevermar),
...
gss <- gss %>% 
  clean_names() %>% 
  rename(age = agec,
         parents_divorced = apardivc,
         parents_separated = aparsepc,
         ever_married = evermar,
         intention_to_marry = wevermar,
         num_marriages = nmarevrc,
         separate_or_divorce = nsepevr) 



#### Clean up ####
gss <- gss %>% 
  mutate_at(vars(age:separate_or_divorce), 
            .funs = funs(ifelse(.=="Valid skip"|.=="Refusal"|.=="Not stated", "NA", .))) 

write_csv(gss, "analysis1.csv")
